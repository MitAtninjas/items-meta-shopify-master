    ...
    jQuery(document).ready(function () {
        ...
        jQuery(document).on("click", ".ajax-submit", function (e) {
            ...
            if (submit_btn.attr("value") && submit_btn.attr("name")) {
                formData.append(
                    submit_btn.attr("name"),
                    submit_btn.attr("value")
                );
            }
            console.log(url);
            ...
            jQuery.ajax({
                ...
                error: function error(data) {
                    jQuery.each(
                        ...
                        function (key, index) {
                            var accordianId = key.split("[")[0];
                            var accordian = jQuery(
                                '[data-id="' + accordianId + '"]'
                            );
                            if (accordian.length) {
                                accordian.focus();
                            ...
                            if (~key.indexOf(".")) {
                                ...
                                if (jQuery("#" + key).parent("td").length) {
                                    ...
                                    if (jQuery(".documentTable tr").length) {
                                        jQuery(".documentTable tr")
                                            .eq(key.split("-")[1])
                                            .css("background-color", "#fdf1ed");
                                    }
                                }
                            } else {
                                var input = jQuery(form).find(
                                    '[name="' + key + '"]'
                                );

                                if (input.length == 0) {
                                    input = jQuery(form).find(
                                        '[name="' + key + '[]"]'
                                    );
                                }
        ...
        jQuery(document).on("click", ".btn-delete, .btn-approve", function (e) {
            e.preventDefault();
            var url = jQuery(this).attr("href");
            var desc = jQuery(this).attr("swal-desc");
            var method = jQuery(this).attr("swal-method");
            var confirmButton = jQuery(this).attr("swal-confirm");
        ...
        jQuery(".border-bottom .content .nav-tabs #nav-item").on(
            "click",
            function () {
                Dashmix.layout("header_loader_on");
            }
        ...
        if (jQuery("#page-header-search-input").length) {
            jQuery("#page-header-search-input")
                .autocomplete({
                    source: function source(request, response) {
                        jQuery.ajax({
                            ...
                            url: jQuery("#page-header-search-input")
                                .parents("form:first")
                                .attr("action"),
                            success: function success(data) {
                                jQuery("#page-header-search-input").removeClass(
